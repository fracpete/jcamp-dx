...
/**
 * Prototype implementation for 2D data ranges.
 * 
 * @author Thomas Weber
 */
public abstract class Range2D extends Range {
	 ...
	 * @author Thomas Weber
	 */
	public static class Int extends Range2D implements Range.Int, Cloneable {
		 ...
		 * <code>y1</code>), max(<code>y0</code>,<code>y1</code>)]
		 * 
		 * @param x0
		 *            int
		 * @param x1
		...
		/**
		 * clip range <code>x0,x1,y0,y1</code>. return <code>null</code> if
		 * range is outside or on border
		 * 
		 * @param x0
		 *            int
		 * @param x1
		...
		/**
		 * set range to [min(<code>x0</code>,<code>x1</code>), max(
		 * <code>x0</code>,<code>x1</code>)] [min(<code>y0</code>,
		 * <code>y1</code>), max(<code>y0</code>,<code>y1</code>)]
		 * 
		 * @param x0
		...
		/**
		 * gets union of [<code>xmin,xmax,ymin,ymax</code> ] with
	 ...
	 * @author Thomas Weber
	 */
	public static class Double extends Range2D implements Range.Double,
			Cloneable {
		...
		public Double(Range1D xrange, Range1D yrange) {
			this(xrange.getXMinAsDouble(), xrange.getXMaxAsDouble(), yrange
					.getXMinAsDouble(), yrange.getXMaxAsDouble());
		}
		...
		public void set(Range1D xrange, Range1D yrange) {
			set(xrange.getXMinAsDouble(), xrange.getXMaxAsDouble(),
					yrange.getXMinAsDouble(), yrange.getXMaxAsDouble());
		}
			...
			if (obj instanceof Range2D) {
				...
				if (this == r
						|| (Math.abs(r.getXMinAsDouble() - xmin) < DOUBLE_EPS
								...
								&& Math.abs(r.getYMinAsDouble() - ymin) < DOUBLE_EPS && Math
								.abs(r.getYMaxAsDouble() - ymax) < DOUBLE_EPS))
					return true;
			}
			return false;
		}
		...
		/**
		 * gets union of <code>(xvalue,yvalue)</code> with <code>this</code>
		 * range.
		 * 
		 * @param xvalue
		 *            double
		 ...
		 * @return Range2D.Double
		 */
		public Range2D.Double union(double xmin, double xmax, double ymin,
				double ymax) {
			Range2D.Double range = (Range2D.Double) clone();
			if (xmin < range.xmin)
				range.xmin = xmin;
		 ...
		 * @return Range2D.Double (this)
		 */
		public Range2D.Double include(double xmin, double xmax, double ymin,
				double ymax) {
			if (xmin < this.xmin)
				this.xmin = xmin;
			if (xmax > this.xmax)
	 ...
	 * @author Thomas Weber
	 */
	public static class Float extends Range2D implements Range.Float, Cloneable {
		...
		public Float(Range1D xrange, Range1D yrange) {
			this((float) xrange.getXMinAsDouble(), (float) xrange
					.getXMaxAsDouble(), (float) yrange.getXMinAsDouble(),
		 ...
		 */
		public void set(Range1D xrange, Range1D yrange) {
			set((float) xrange.getXMinAsDouble(),
		...
		public Float(Range1D.Float xrange, Range1D.Float yrange) {
			this(xrange.getXMin(), xrange.getXMax(), yrange.getXMin(), yrange
					.getXMax());
		}
		...
		public void set(Range1D.Float xrange, Range1D.Float yrange) {
			set(xrange.getXMin(), xrange.getXMax(), yrange.getXMin(),
					yrange.getXMax());
		}
			...
			if (obj instanceof Range2D) {
				...
				if (this == r
						|| (Math.abs(r.getXMinAsDouble() - xmin) < FLOAT_EPS
								...
								&& Math.abs(r.getYMinAsDouble() - ymin) < FLOAT_EPS && Math
								.abs(r.getYMaxAsDouble() - ymax) < FLOAT_EPS))
					return true;
			}
			return false;
		}
		...
		public boolean equals(Range2D.Float r) {
			if (this == r
					|| (Math.abs(r.xmin - xmin) < FLOAT_EPS
							...
							&& Math.abs(r.ymin - ymin) < FLOAT_EPS && Math
							.abs(r.ymax - ymax) < FLOAT_EPS))
				return true;
			return false;
		}
		 ...
		 * @return Range2D.Float
		 */
		public Range2D.Float union(float xmin, float xmax, float ymin,
				float ymax) {
			Range2D.Float range = (Range2D.Float) clone();
			if (xmin < range.xmin)
				range.xmin = xmin;
		 ...
		 * @return Range2D.Float (this)
		 */
		public Range2D.Float include(float xmin, float xmax, float ymin,
				float ymax) {
			if (xmin < this.xmin)
				this.xmin = xmin;
			if (xmax > this.xmax)
	...
	public boolean contains(Range2D r) {
		return (r.getXMinAsDouble() >= getXMinAsDouble()
				...
				&& r.getYMinAsDouble() >= getYMinAsDouble() && r
				.getYMaxAsDouble() <= getYMaxAsDouble());
	}
