...
public class ASDFParser extends antlr.LLkParser implements ASDFParserTokenTypes {
	...
	protected final void block(int expectedSize) throws RecognitionException,
			TokenStreamException, JCAMPException {
			...
			_loop38: do {
				if ((LA(1) == XCHECK || LA(1) == WS)) {
					...
					if (lastInDIF) {
						...
						state.setCheckY(lineContent.yVector.elementAt(0)
								.doubleValue());
						state.setCurrentY(yValues.lastElement().intValue());
					} else {
						// second element is a real data point
						// insert it into data point array and increase index
						Double y0 = lineContent.yVector.elementAt(0);
					...
					case ASDFCheckHandler.ABORT: {
						error("ASDF parsing aborted, failed check in line "
								+ Integer
										.toString(state.getCurrentLineNumber()));
					}
					case ASDFCheckHandler.REPLACE: {
						// compare indices
						int indexAfterCheck = state.getCurrentIndex();
						if (indexBeforeCheck < indexAfterCheck) {
							// if new index is greater, insert error values
							// (last value so far)
							Double errorValue = yValues.lastElement();
							...
							yValues.setElementAt(
									new Double(state.getCurrentY()),
									indexAfterCheck);
						} else {
	...
	protected final int difNumber() throws RecognitionException,
	...
	protected final Vector<Double> difNumbers() throws RecognitionException,
			TokenStreamException {
		Vector<Double> v;
				...
				case NEGDIF: {
					{
						...
						currentInDIF = true;
						double l = v.lastElement().doubleValue();
				...
				case DUP: {
					{
						...
						if (currentInDIF) {
							...
							double l0 = v.elementAt(l - 1).doubleValue();
							double l1 = v.elementAt(l - 2).doubleValue();
							double dif = l0 - l1;
	...
	protected final int dupNumber() throws RecognitionException,
			TokenStreamException {
		int v;
	...
	public double[] getYData() {
		...
		for (int i = 0; i < n; i++) {
			y[i] = yValues.elementAt(i).doubleValue();
		}
		return y;
	}

	protected final LineContent line() throws RecognitionException,
			TokenStreamException, JCAMPException {
	...
	protected final Vector<Double> pacNumbers() throws RecognitionException,
			TokenStreamException {
		Vector<Double> v;
	...
	/**
	 * sets active x-sequence/y-value check handler to <code>handler</code> if
	 * null, an ignoring handler is set
	 */
	...
	protected final int sqzNumber() throws RecognitionException,
			TokenStreamException {
		int v;
	...
	protected final double xcheck() throws RecognitionException,
			TokenStreamException {
		double d;
