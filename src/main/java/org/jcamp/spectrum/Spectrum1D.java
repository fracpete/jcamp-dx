...
public class Spectrum1D extends Spectrum implements Cloneable {
	...
	protected void adjustFullViewRange() {
		...
		if (isFullSpectrum()) {
			...
			if (n > 1) {
				Range1D.Double xrange = xData.getRange1D();
				w = xrange.getXWidth();
				xrange.set(xrange.getXMin() - .05 * w, xrange.getXMax() + .05
						* w);
				setXFullViewRange(xrange);
			} else if (n == 1) {
			...
			if (n > 1) {
				Range1D.Double yrange = yData.getRange1D();
				yrange.set(Math.min(yrange.getXMin(), 0.0),
						Math.max(yrange.getXMax(), 0.0));
				w = yrange.getXWidth();
	...
	public Object clone() {
		...
		spectrum.setData((IOrderedDataArray1D) xData.clone(),
				(IDataArray1D) yData.clone());
	...
	public void removeAssignmentAt(double pos) {
		if (this.assignments != null && this.assignments.length > 0) {
			...
			System.arraycopy(this.assignments, index + 1, newTable, index,
					this.assignments.length - index - 1);
	...
	public void removePatternAt(double pos) {
		if (this.patternTable != null && this.patternTable.length > 0) {
			...
			System.arraycopy(this.patternTable, index + 1, newTable, index,
					this.patternTable.length - index - 1);
	...
	public void removePeakAt(double pos) {
		if (this.peakTable != null && this.peakTable.length > 0) {
			...
			System.arraycopy(this.peakTable, index + 1, newTable, index,
					this.peakTable.length - index - 1);
